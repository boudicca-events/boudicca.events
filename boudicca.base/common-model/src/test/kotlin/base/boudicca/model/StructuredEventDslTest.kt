...
class StructuredEventDslTest {
    ...
    fun `event dsl can be used to create structured event`() {
        val event =
            structuredEvent("testEvent", OffsetDateTime.now()) {
    ...
    fun `event dsl compact form can be used to create structured event with less lines`() {
        ...
        assertAll {
            ...
            assertThat(
                event.filterKeys(KeyFilter.parse("description:format=markdown")).first().second,
            ).isEqualTo("# mydata")
            assertThat(
                event.filterKeys(KeyFilter.parse("sources")).first().second,
