...
internal class Guesser(private val tokenGroups: List<List<Pair<TokenizerType, String>>>) {
    ...
    private fun buildChain(): MutatorChain {
        return FixedPatternStep(CheckIfSolvableStep(UntilStep(CheckIfSolvableStep(GroupingStep(CheckIfSolvableStep(null))))))
    }
    ...
    class UntilStep(val next: MutatorChain) : MutatorChain {
        override fun mutate(guess: Guess): DateParserResult? {
            var result = next.mutate(guess)
            if (result != null) {
                return result
...
internal class Guesses(val guesses: List<Component>) : Guess {
    ...
    override fun solve(): DateParserResult? {
        ...
        return buildSingleDateParserResultFromComponents(components)
    }
...
internal data class Grouping(val inputGroups: List<Guesses>) : Grouper {
    override fun solve(): DateParserResult? {
        ...
        return buildSingleDateParserResultFromComponents(leftComponents + rightComponents)
    }
...
internal data class Until(val inputGroups: List<Guesses>) : Grouper {
    override fun solve(): DateParserResult? {
        val groups = inputGroups.filter { it.isInteresting() }
        if (groups.size == 1) {
            return groups.single().solve()
        ...
        val leftDate = buildOffsetDateTimeFromComponents(leftFull)
        val rightDate = buildOffsetDateTimeFromComponents(rightFull)
...
private fun buildSingleDateParserResultFromComponents(components: List<SolutionComponent>): DateParserResult? {
    return buildOffsetDateTimeFromComponents(components)?.run { DateParserResult(listOf(DatePair(this))) }
}

private fun buildOffsetDateTimeFromComponents(components: List<SolutionComponent>): OffsetDateTime? {
