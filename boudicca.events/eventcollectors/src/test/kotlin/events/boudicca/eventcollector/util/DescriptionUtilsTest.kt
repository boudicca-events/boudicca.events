...
class DescriptionUtilsTest {
    ...
    @Test
    fun testHtmlToMarkdownNoImage() {
        htmlToMarkdownNoImageAssert("Hello", "<span>Hello</span>")
        htmlToMarkdownNoImageAssert("", "<img src=\"https://example.com\" title=\"Image\" />")
    }

    private fun htmlToTextAssert(
        text: String,
        html: String,
    ) {
    ...
    private fun htmlToMarkdownAssert(
        markdown: String,
        html: String,
    ) {
        assertEquals(markdown, htmlToDescriptionMarkdown(Jsoup.parse(html)))
    }

    private fun htmlToMarkdownNoImageAssert(
        markdown: String,
        html: String,
    ) {
        assertEquals(markdown, htmlToDescriptionMarkdown(Jsoup.parse(html), includeImage = false))
    }
}
