...
class OsmEnricher(
    ...
    private fun searchOsmId(event: StructuredEvent): String? {
        val locationNames = event.getProperty(SemanticKeys.LOCATION_NAME_PROPERTY).map(Pair<Key, String>::second)
        ...
        val query =
            """$NOMINATIM_BASE_URL/search?accept-language=de,en&format=jsonv2&limit=1&q=${locationQuery.encodeURL()}""".trim()
