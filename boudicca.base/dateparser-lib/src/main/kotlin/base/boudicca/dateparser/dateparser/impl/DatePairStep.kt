...
internal class DatePairStep(
    ...
    fun solve(): DatePairSolution? {
        ...
        for (untilIndex in untilIndexes) {
            ...
            val tokensList =
                listOf(
                    Tokens(tokens.tokens.take(untilIndex)),
                    Tokens(tokens.tokens.drop(untilIndex + 1)),
        ...
        val result =
            trySolve(
                debugTracing.startOperationWithChild("trying no until handling", tokens),
                listOf(
                    tokens,
                ),
            )
    ...
    private fun findAllUntilIndexes(tokens: Tokens): List<Int> =
        tokens.tokens
            .mapIndexed { i, component -> Pair(i, component) }
    ...
    private fun trySolve(
        ...
        if (leftDateSolution == null ||
            rightDateSolution == null ||
