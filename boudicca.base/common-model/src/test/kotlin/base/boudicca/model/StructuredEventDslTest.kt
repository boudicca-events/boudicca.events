...
class StructuredEventDslTest {
    ...
    fun `event dsl can be used to create structured event`() {
        ...
        assertThat(event.filterKeys(Key.parse("sources:format=list")).first().second).isEqualTo("asdf.com,bsdf.com,csdf.com")
    }
    ...
    fun `event can be modified with event dsl`() {
        ...
        assertThat(modifiedEvent.filterKeys(Key.parse("description:format=markdown")).first().second).isEqualTo("# mydata")
