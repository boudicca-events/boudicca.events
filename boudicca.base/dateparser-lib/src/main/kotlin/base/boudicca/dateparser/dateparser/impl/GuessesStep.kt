...
internal class GuessesStep(
    private val config: DateParserConfig, private val debugTracing: DebugTracing, private val tokens: Tokens
) {
    fun solve(): ListOfDatePairSolution? {
        ...
        if (result == null) {
            result = ListOfDatePairStep(
                config, debugTracing.startOperationWithChild("applied no guesses", tokens), tokens
            ).solve()
            debugTracing.endOperation(result)
        }
